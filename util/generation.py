...
def adjust_probs(
    ...
    is_head = text_list[-1] == tokens.BEGIN_TOKEN_STR or text_list[-1][0] == tokens.TRACK_EVENTS_CHAR
    is_sep = text_list[-1] == tokens.SEP_TOKEN_STR
    if is_head: # if the section is head and not sep
        if len(text_list) == 1:
            ...
            used_track_number =  [
                b36str2int(text_list[i].split(':')[1]) + 1 # the id:value of track number is 0:padding, 1:0, 2:1, ...
        ...
        if text_list[-1][0] == tokens.MEASURE_EVENTS_CHAR:
        ...
        elif text_list[-1][0] == tokens.POSITION_EVENTS_CHAR:
        ...
        elif text_list[-1][0] in (tokens.NOTE_EVENTS_CHAR, tokens.MULTI_NOTE_EVENTS_CHAR):
            probs[ATTR_NAME_INDEX['evt']][list(tempo_indices)] = 0

        # this prohibits the position tokens that is "behind" the current position
        k = len(text_list) - 1
        while text_list[k][0] not in (tokens.POSITION_EVENTS_CHAR, tokens.MEASURE_EVENTS_CHAR) and k > 0:
            k -= 1
        if text_list[k][0] == tokens.POSITION_EVENTS_CHAR:
            cur_position = b36str2int(text_list[k][1:])
        ...
        k = len(text_list) - 1
        while text_list[k][0] not in (tokens.POSITION_EVENTS_CHAR, tokens.TEMPO_EVENTS_CHAR) and k > 0:
            k -= 1
        if text_list[k][0] == tokens.TEMPO_EVENTS_CHAR:
            probs[ATTR_NAME_INDEX['evt']][list(tempo_indices)] = 0

        # only allow the defined track number
        try:
            sep_index_in_text_list = text_list.index(tokens.SEP_TOKEN_STR)
            used_track_number_indices = {
                b36str2int(t.split(':')[1]) + 1
                for t in text_list[1:sep_index_in_text_list]
...
def generate_piece(
    ...
    for _ in tqdm(range(max_gen_step), disable=not show_tqdm, leave=None):
        ...
        while try_count < try_count_limit:
            ...
            input_seq = torch.from_numpy(try_array.astype('int32')).unsqueeze(0).int() # shape = (1, seq_length, all_attr_num)
