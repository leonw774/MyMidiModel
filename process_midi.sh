...
python3 data/midi_2_text.py --nth ${NTH} --max-track-number ${MAX_TRACK_NUMBER} --max-duration ${MAX_DURATION} \
    --velocity-step ${VELOCITY_STEP} --tempo-quantization ${TEMPO_MIN} ${TEMPO_MAX} ${TEMPO_STEP} -r -w ${PROCESS_WORKERS} \
    -o data/processed_midi/corpus_${PARAS_SUFFIX}.txt ${MIDI_DIR_PATH}
