...
def merge_sparse_track(
        midi: MidiFile,
        sparse_threshold_ratio: float = 0.05,
        sparse_threshold_max: int = 50,
        discard_threshold: int = 1) -> None:
...
def get_time_structure_tokens(
    ...
    if len(time_signature_changes) == 0:
        time_sig_tuple_list = [(4, 4, 0)] # (numerator, denominator, change_time)
    else:
        time_sig_tuple_list = []
        ...
        for i, time_sig in enumerate(time_signature_changes):
            ...
            if len(time_sig_tuple_list) == 0:
                time_sig_tuple_list.append(time_sig_tuple)
            elif (time_sig_tuple[0] != time_sig_tuple_list[-1][0]
                    or time_sig_tuple[1] != time_sig_tuple_list[-1][1]):
                time_sig_tuple_list.append(time_sig_tuple)
    ...
    if time_sig_tuple_list[0][2] == 1:
        if len(time_signature_changes) > 1:
            nth_per_measure = round(nth_per_beat * time_sig_tuple_list[0][0] * (4 / time_sig_tuple_list[0][1]))
            if time_sig_tuple_list[1][2] % nth_per_measure == 0:
                time_sig_tuple_list[0] = (time_sig_tuple_list[0][0], time_sig_tuple_list[0][1], 0)
    ...
    for i, (n, d, cur_timesig_start_time) in enumerate(time_sig_tuple_list):
        ...
        # check if the difference of starting time between current and next time signatures are multiple of current measure's length
    ...
    tempo_token_list = []
    tempo_tuple_list = [] # (bpm, change_time)
    if len(midi.tempo_changes) == 0:
        tempo_tuple_list = [(120, 0)]
    else:
        prev_change_time = None # to remove duplicate change times
        for i, tempo_change in enumerate(midi.tempo_changes):
            ...
            if prev_change_time != change_time:
                bpm = quantize_tempo(tempo_change.tempo, tempo_quantization)
                tempo_tuple_list.append((bpm, change_time))
            prev_change_time = change_time
    # print(tempo_tuple_list)

    # same issue as time signature, but tempo can appear in any place so this is all I can do
    if tempo_tuple_list[0][1] == 1 and time_sig_tuple_list[0][2] == 0:
        tempo_tuple_list[0][1] == 0

    tempo_token_list = []
    # use tempo token to record tempo info for `get_position_token` to handle
    # also remove the tempo changes that didn't impacted the bpm after quantification
    prev_bpm = None
    for i, (bpm, change_time) in enumerate(tempo_tuple_list):
        if prev_bpm != bpm:
...
def get_position_tokens(note_measure_tokens_list: list, tempo_token_list: list, tempo_method: str) -> list:
    ...
    for i, token in enumerate(note_measure_tokens_list):

        if token.type_priority == TYPE_PRIORITY['MeasureToken']:
            cur_measure_onset_time = token.onset

        if tempo_method == 'position_event' and tempo_cursor < len(tempo_token_list):
            # there are some tempo changes without notes between them
            # which is weird and unnecessary but there are part of the data so have to keep it
            while tempo_token_list[tempo_cursor].onset <= token.onset and tempo_cursor < len(tempo_token_list):
                try:
                    ...
                    print('negtive position:', tempo_cursor)
                    print('cur_measure_onset_time:', cur_measure_onset_time)
                    print('token.onset:', token.onset)
                    print('\n'.join(map(str, tempo_token_list)))
                    raise e
...
def midi_to_text_list(
        ...
        use_merge_sparse: bool = True) -> list:
    """
        Parameters:
        ...
        - tempo_method: can be 'position_attribute' or 'position_event'
          - 'attribute' means tempo info is part of the position token
    ...
    assert tempo_method in ['position_attribute', 'position_event'], \
...
def handle_note_continuation(
    ...
    if cur_time_in_ticks in pending_cont_notes:
        if info in pending_cont_notes[cur_time_in_ticks]:
            ...
            #     print(cur_time_in_ticks, 'onset', onset, 'pop', pending_cont_notes)
            # else:
            #     print(cur_time_in_ticks, 'onset no pop', pending_cont_notes)
            if len(pending_cont_notes[cur_time_in_ticks][info]) == 0:
                pending_cont_notes[cur_time_in_ticks].pop(info)
            if len(pending_cont_notes[cur_time_in_ticks]) == 0:
    ...
    if is_cont:
        ...
        pending_cont_time = cur_time_in_ticks + duration
        ...
        # print(cur_time_in_ticks, 'onset, onset, append', pending_cont_notes)
    else:
        return Note(velocity, pitch, onset, cur_time_in_ticks + duration)
    return None


def piece_to_midi(piece: str, nth: int) -> MidiFile:
    midi = MidiFile(ticks_per_beat=nth//4) # tick time == nth time
    ...
    for text in text_list[1:-1]:
        ...
        if not is_head:
            if typename == 'M':
                ...
                cur_measure_length_in_ticks = 4 * numer * round(midi.ticks_per_beat / denom)
                if cur_time_signature != (numer, denom):
                    cur_time_signature = (numer, denom)
                    midi.time_signature_changes.append(TimeSignature(numerator=numer, denominator=denom, time=cur_time_in_ticks))

                if len(tempo_change):
                    if cur_tempo != tempo_change[0]:
                        midi.tempo_changes.append(TempoChange(tempo=tempo_change[0], time=cur_time_in_ticks))
                ...
                cur_time_in_ticks = time_position + cur_measure_onset_in_ticks
                if len(tempo_change):
                    if cur_tempo != tempo_change[0]:
                ...
                n = handle_note_continuation(is_cont, note_attr, cur_time_in_ticks, pending_cont_notes)
                if n is not None:
                    midi.instruments[note_attr[3]].notes.append(n)
                debug_str += text + ' onset' + str(cur_time_in_ticks) + ' ' + str(n) + '\n'
                ...
                base_pitch, time_unit, velocity, track_number = (int(x, TOKEN_INT2STR_BASE) for x in puvt)
                debug_str += text
                for is_cont, rel_onset, rel_pitch, rel_dur in relnote_list:
                    ...
                    onset_time_in_tick = cur_time_in_ticks + rel_onset * time_unit
    ...
    with open('peice_to_midi_debug_str.txt', 'w+') as debugfile:
            ...
            for n in inst.notes:
                if n.start in pending_cont_notes:
                    ...
                    if info in pending_cont_notes[n.start]:
                        ...
                        new_note = handle_note_continuation(False, (n.pitch, n.end-n.start, n.velocity, track_number), n.start, pending_cont_notes)
...
def parse_corpus_paras_yaml(yaml: str) -> dict:
    ...
    for p in pairs:
        ...
        if k == 'tempo_quantization':
            ...
            args_dict[k] = v
        elif k == 'use_merge_drums' or k == 'use_merge_sparse':
            args_dict[k] = (v == 'True')
        else:
            args_dict[k] = int(v)
    return args_dict

def file_to_corpus_paras_and_piece_iterator(corpus_file: IOBase):
    yaml_string = ''
    corpus_file.seek(0)
    ...
    return corpus_paras, PieceIterator(corpus_file)
